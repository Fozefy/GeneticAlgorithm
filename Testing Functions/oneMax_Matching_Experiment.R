graph = gridConstructor(100) #4 connections
coevoSpt.matching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withMatching(), goal=45, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=15), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.matching[i] = ga$gen
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.matching,file="coevoSpt.matching")

graph = complete.graph(100) #complete connections
coevo.matching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withMatching(), goal=45, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=15),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.matching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.matching,file="coevo.matching")

graph = complete.graph(100)
std.matching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withMatching(),goal=45), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.matching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.matching,file="std.matching")

graph = gridConstructor(100)
stdSpatial.matching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withMatching(),goal=45), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.matching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.matching,file="stdSpatial.matching")

#More matching

graph = gridConstructor(100) #4 connections
coevoSpt.extraMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=16), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.extraMatching[i] = ga$gen
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.extraMatching,file="coevoSpt.extraMatching")

graph = complete.graph(100) #complete connections
coevo.extraMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=16),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.extraMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.extraMatching,file="coevo.extraMatching.a")

graph = complete.graph(100)
std.extraMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withMatching(matching=4),goal=90), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.extraMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.extraMatching,file="std.extraMatching")

graph = gridConstructor(100)
stdSpatial.extraMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withMatching(matching=4),goal=90), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.extraMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.extraMatching,file="stdSpatial.extraMatching")


#Pure Matching
graph = gridConstructor(100) #4 connections
coevoSpt.pureMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withMatching(primary=0,secondary=0,matching=1), goal=15, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=15), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.pureMatching[i] = ga$gen
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.pureMatching,file="coevoSpt.pureMatching")

graph = complete.graph(100) #complete connections
coevo.pureMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withMatching(primary=0,secondary=0,matching=1), goal=15, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=15),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.pureMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.pureMatching,file="coevo.pureMatching")

graph = complete.graph(100)
std.pureMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withMatching(primary=0,secondary=0,matching=1), goal=15), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.pureMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.pureMatching,file="std.pureMatching")

graph = gridConstructor(100)
stdSpatial.pureMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withMatching(primary=0,secondary=0,matching=1), goal=15), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.pureMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.pureMatching,file="stdSpatial.pureMatching")

#Inner Matching
graph = gridConstructor(100) #4 connections
coevoSpt.PureInnerMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(primary=0,secondary=0,matching=1), goal=15, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=15), verbose=FALSE,reporting.fn=reportNone.report.fn)
generational.ga(ga)
coevoSpt.PureInnerMatching[i] = ga$gen
print(paste(i,"Complete"))
rm(ga)
}
save(coevoSpt.PureInnerMatching,file="coevoSpt.PureInnerMatching")

graph = complete.graph(100) #complete connections
coevo.PureInnerMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(primary=0,secondary=0,matching=1), goal=15, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=15),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.PureInnerMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.PureInnerMatching,file="coevo.PureInnerMatching")

graph = complete.graph(100)
std.PureInnerMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(primary=0,secondary=0,matching=1), goal=15), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.PureInnerMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.PureInnerMatching,file="std.PureInnerMatching")

graph = gridConstructor(100)
stdSpatial.PureInnerMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(primary=0,secondary=0,matching=1), goal=15), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.PureInnerMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.PureInnerMatching,file="stdSpatial.PureInnerMatching")

#Extra Inner Matching
graph = gridConstructor(100) #4 connections
coevoSpt.ExtraInnerMatching.FullElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=100,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=16), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.ExtraInnerMatching.FullElite[i] = ga$gen
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.ExtraInnerMatching.FullElite,file="coevoSpt.ExtraInnerMatching.FullElite")

graph = complete.graph(100) #complete connections
coevo.ExtraInnerMatching.FullElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=100,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=16),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.ExtraInnerMatching.FullElite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.ExtraInnerMatching.FullElite,file="coevo.ExtraInnerMatching.FullElite")

graph = complete.graph(100)
std.ExtraInnerMatching.FullElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=100,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.ExtraInnerMatching.FullElite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.ExtraInnerMatching.FullElite,file="std.ExtraInnerMatching.FullElite")

graph = gridConstructor(100)
stdSpatial.ExtraInnerMatching.FullElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=100,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.ExtraInnerMatching.FullElite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.ExtraInnerMatching.FullElite,file="stdSpatial.ExtraInnerMatching.FullElite")

graph = gridConstructor(100) #4 connections
coevoSpt.ExtraInnerMatching.2Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=16), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.ExtraInnerMatching.2Elite[i] = ga$gen
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.ExtraInnerMatching.2Elite,file="coevoSpt.ExtraInnerMatching.2Elite")

graph = complete.graph(100) #complete connections
coevo.ExtraInnerMatching.2Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=16),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.ExtraInnerMatching.2Elite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.ExtraInnerMatching.2Elite,file="coevo.ExtraInnerMatching.2Elite")

graph = complete.graph(100)
std.ExtraInnerMatching.2Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.ExtraInnerMatching.2Elite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.ExtraInnerMatching.2Elite,file="std.ExtraInnerMatching.2Elite")

graph = gridConstructor(100)
stdSpatial.ExtraInnerMatching.2Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.ExtraInnerMatching.2Elite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.ExtraInnerMatching.2Elite,file="stdSpatial.ExtraInnerMatching.2Elite")

#HalfElite Inner MAtching
graph = gridConstructor(100) #4 connections
coevoSpt.ExtraInnerMatching.50Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=50,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=16), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.ExtraInnerMatching.50Elite[i] = ga$gen
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.ExtraInnerMatching.50Elite,file="coevoSpt.ExtraInnerMatching.50Elite")

graph = complete.graph(100) #complete connections
coevo.ExtraInnerMatching.50Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=50,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=16),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.ExtraInnerMatching.50Elite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.ExtraInnerMatching.50Elite,file="coevo.ExtraInnerMatching.50Elite")

graph = complete.graph(100)
std.ExtraInnerMatching.50Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=50,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.ExtraInnerMatching.50Elite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.ExtraInnerMatching.50Elite,file="std.ExtraInnerMatching.50Elite")

graph = gridConstructor(100)
stdSpatial.ExtraInnerMatching.50Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=50,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.ExtraInnerMatching.50Elite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.ExtraInnerMatching.50Elite,file="stdSpatial.ExtraInnerMatching.50Elite")

#Inner MAtching 10 Elite
graph = gridConstructor(100) #4 connections
coevoSpt.ExtraInnerMatching.10Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=10,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=16), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.ExtraInnerMatching.10Elite[i] = ga$gen
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.ExtraInnerMatching.10Elite,file="coevoSpt.ExtraInnerMatching.10Elite")

graph = complete.graph(100) #complete connections
coevo.ExtraInnerMatching.10Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=10,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=16),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.ExtraInnerMatching.10Elite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.ExtraInnerMatching.10Elite,file="coevo.ExtraInnerMatching.10Elite")

graph = complete.graph(100)
std.ExtraInnerMatching.10Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=10,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.ExtraInnerMatching.10Elite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.ExtraInnerMatching.10Elite,file="std.ExtraInnerMatching.10Elite")

graph = gridConstructor(100)
stdSpatial.ExtraInnerMatching.10Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=10,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.ExtraInnerMatching.10Elite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.ExtraInnerMatching.10Elite,file="stdSpatial.ExtraInnerMatching.10Elite")

#Inner MAtching 1 Elite
graph = gridConstructor(100) #4 connections
coevoSpt.ExtraInnerMatching.1Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=1,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=16), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.ExtraInnerMatching.1Elite[i] = ga$gen
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.ExtraInnerMatching.1Elite,file="coevoSpt.ExtraInnerMatching.1Elite")

graph = complete.graph(100) #complete connections
coevo.ExtraInnerMatching.1Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=1,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=16),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.ExtraInnerMatching.1Elite[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.ExtraInnerMatching.1Elite,file="coevo.ExtraInnerMatching.1Elite")

graph = complete.graph(100)
std.ExtraInnerMatching.1Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=1,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.ExtraInnerMatching.1Elite[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.ExtraInnerMatching.1Elite,file="std.ExtraInnerMatching.1Elite")

graph = gridConstructor(100)
stdSpatial.ExtraInnerMatching.1Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=1,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.ExtraInnerMatching.1Elite[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.ExtraInnerMatching.1Elite,file="stdSpatial.ExtraInnerMatching.1Elite")

#Extra Inner Matching No-Elite
graph = gridConstructor(100) #4 connections
coevoSpt.ExtraInnerMatching.NoElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=FALSE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=16), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.ExtraInnerMatching.NoElite[i] = ga$gen
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.ExtraInnerMatching.NoElite,file="coevoSpt.ExtraInnerMatching.NoElite")

graph = complete.graph(100) #complete connections
coevo.ExtraInnerMatching.NoElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=FALSE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=16),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.ExtraInnerMatching.NoElite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.ExtraInnerMatching.NoElite,file="coevo.ExtraInnerMatching.NoElite")

graph = complete.graph(100)
std.ExtraInnerMatching.NoElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=FALSE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.ExtraInnerMatching.NoElite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.ExtraInnerMatching.NoElite,file="std.ExtraInnerMatching.NoElite")

graph = gridConstructor(100)
stdSpatial.ExtraInnerMatching.NoElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=FALSE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.ExtraInnerMatching.NoElite[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.ExtraInnerMatching.NoElite,file="stdSpatial.ExtraInnerMatching.NoElite")

#Extra Inner Matching - 5 Elite
graph = gridConstructor(100) #4 connections
coevoSpt.ExtraInnerMatching.5Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=5,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=16), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.ExtraInnerMatching.5Elite[i] = ga$gen
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.ExtraInnerMatching.5Elite,file="coevoSpt.ExtraInnerMatching.5Elite")

graph = complete.graph(100) #complete connections
coevo.ExtraInnerMatching.5Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(matching=4), goal=96, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=5,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=16),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.ExtraInnerMatching.5Elite[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.ExtraInnerMatching.5Elite,file="coevo.ExtraInnerMatching.5Elite")

graph = complete.graph(100)
std.ExtraInnerMatching.5Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=5,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.ExtraInnerMatching.5Elite[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.ExtraInnerMatching.5Elite,file="std.ExtraInnerMatching.5Elite")

graph = gridConstructor(100)
stdSpatial.ExtraInnerMatching.5Elite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(matching=4), goal=96), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,elite.size=5,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.ExtraInnerMatching.5Elite[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.ExtraInnerMatching.5Elite,file="stdSpatial.ExtraInnerMatching.5Elite")


#Inner Matching Mix
graph = gridConstructor(100) #4 connections
coevoSpt.InnerMatchingMix = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(primary=1,secondary=1,matching=1), goal=45, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=15), verbose=FALSE,reporting.fn=reportNone.report.fn)
generational.ga(ga)
coevoSpt.InnerMatchingMix[i] = ga$gen
print(paste(i,"Complete"))
rm(ga)
}
save(coevoSpt.InnerMatchingMix,file="coevoSpt.InnerMatchingMix")

graph = complete.graph(100) #complete connections
coevo.InnerMatchingMix = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerMatching(primary=1,secondary=1,matching=1), goal=45, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=15),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.InnerMatchingMix[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.InnerMatchingMix,file="coevo.InnerMatchingMix")

graph = complete.graph(100)
std.InnerMatchingMix = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(primary=1,secondary=1,matching=1), goal=45), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.InnerMatchingMix[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.InnerMatchingMix,file="std.InnerMatchingMix")

graph = gridConstructor(100)
stdSpatial.InnerMatchingMix = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerMatching(primary=1,secondary=1,matching=1), goal=45), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.InnerMatchingMix[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.InnerMatchingMix,file="stdSpatial.InnerMatchingMix")

#Anti Matching Mix
graph = gridConstructor(100) #4 connections
coevoSpt.AntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withAntiMatching(), goal=31, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=15), verbose=FALSE,reporting.fn=reportNone.report.fn)
generational.ga(ga)
coevoSpt.AntiMatching[i] = ga$gen
print(paste(i,"Complete -", ga$gen))
rm(ga)
}
save(coevoSpt.AntiMatching,file="coevoSpt.AntiMatching")

graph = complete.graph(100) #complete connections
coevo.AntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withAntiMatching(), goal=31, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=15),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.AntiMatching[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.AntiMatching,file="coevo.AntiMatching")

graph = complete.graph(100)
std.AntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withAntiMatching(), goal=31), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.AntiMatching[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.AntiMatching,file="std.AntiMatching")

graph = gridConstructor(100)
stdSpatial.AntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withAntiMatching(), goal=31), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.AntiMatching[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.AntiMatching,file="stdSpatial.AntiMatching")

#Inner AntiMatching
graph = gridConstructor(100) #4 connections
coevoSpt.InnerAntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerAntiMatching(), goal=33, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=16), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.InnerAntiMatching[i] = ga$gen
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.InnerAntiMatching,file="coevoSpt.InnerAntiMatching")

graph = complete.graph(100) #complete connections
coevo.InnerAntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerAntiMatching(), goal=33, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=16),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.InnerAntiMatching[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.InnerAntiMatching,file="coevo.InnerAntiMatching")

graph = complete.graph(100)
std.InnerAntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerAntiMatching(), goal=33), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.InnerAntiMatching[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.InnerAntiMatching,file="std.InnerAntiMatching")

graph = gridConstructor(100)
stdSpatial.InnerAntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerAntiMatching(), goal=33), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.InnerAntiMatching[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.InnerAntiMatching,file="stdSpatial.InnerAntiMatching")

#Inner Anti Matching /no Elite
graph = gridConstructor(100) #4 connections
coevoSpt.InnerAntiMatching.NoElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerAntiMatching(), goal=33, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=FALSE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=16), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.InnerAntiMatching.NoElite[i] = ga$gen
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.InnerAntiMatching.NoElite,file="coevoSpt.InnerAntiMatching.NoElite")

graph = complete.graph(100) #complete connections
coevo.InnerAntiMatching.NoElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withInnerAntiMatching(), goal=33, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=FALSE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=16),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.InnerAntiMatching.NoElite[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.InnerAntiMatching.NoElite,file="coevo.InnerAntiMatching.NoElite")

graph = complete.graph(100)
std.InnerAntiMatching.NoElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerAntiMatching(), goal=33), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=FALSE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.InnerAntiMatching.NoElite[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.InnerAntiMatching.NoElite,file="std.InnerAntiMatching.NoElite")

graph = gridConstructor(100)
stdSpatial.InnerAntiMatching.NoElite = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withInnerAntiMatching(), goal=33), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=FALSE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=32),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.InnerAntiMatching.NoElite[i] = ga$gen
  
  print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.InnerAntiMatching.NoElite,file="stdSpatial.InnerAntiMatching.NoElite")

#Pure Anti Matching
graph = gridConstructor(100) #4 connections
coevoSpt.PureAntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withAntiMatching(primary=0,secondary=0, matching=1), goal=15, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph),encoding.args=new.encoding.args(chr.length=15), verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  coevoSpt.PureAntiMatching[i] = ga$gen
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevoSpt.PureAntiMatching,file="coevoSpt.PureAntiMatching")

graph = complete.graph(100) #complete connections
coevo.PureAntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000,numPop=2), fitness.args=new.fitness.args(fitness.fn=twoPop.one.max.withAntiMatching(primary=0,secondary=0, matching=1), goal=15, externalConnectionsMatrix=matrix(c(1:100, 1:100), nrow=100, ncol=2)), xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=15),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  coevo.PureAntiMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(coevo.PureAntiMatching,file="coevo.PureAntiMatching")

graph = complete.graph(100)
std.PureAntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withAntiMatching(primary=0,secondary=0, matching=1), goal=15), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  std.PureAntiMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(std.PureAntiMatching,file="std.PureAntiMatching")

graph = gridConstructor(100)
stdSpatial.PureAntiMatching = c(1)
for (i in 1:n)
{
  ga = new.GA.env(GA.base.args=new.GA.base.args(max.gen=5000), fitness.args=new.fitness.args(fitness.fn=onePop.one.max.withAntiMatching(primary=0,secondary=0, matching=1), goal=15), mutation.args = new.mutation.args(prob.mutation=4),xover.args = new.xover.args(keepSecondaryParent=FALSE), selection.args=new.selection.args(elitism=TRUE,adjMatrix=graph), encoding.args=new.encoding.args(chr.length=30),verbose=FALSE,reporting.fn=reportNone.report.fn)
  generational.ga(ga)
  
  stdSpatial.PureAntiMatching[i] = ga$gen
  
print(paste(i,"Complete -", ga$gen))
  rm(ga)
}
save(stdSpatial.PureAntiMatching,file="stdSpatial.PureAntiMatching")