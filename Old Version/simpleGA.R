...
#gene.values <- function(object){object@gene.values}
#alphabet <- gene.values
#setGeneric("gene.values")
#setGeneric("alphabet")
...
setClass("mutation.controller", 
	representation(prob.mutation = "numeric", 
						 alleles.fn = "function", 
						 not.equal.op = "function"))

setMethod("initialize", 
			  signature = "mutation.controller",
          definition = function(.Object, prob.mutation, alleles.fn, not.equal = `!=`, ...) {
            .Object@prob.mutation <- prob.mutation
            .Object@alleles.fn <- alleles.fn
            .Object@not.equal.op <- not.equal
            .Object
          })

prob.mutation <- function(mut.parm){mut.parm@prob.mutation}
setGeneric("prob.mutation")
new.alleles <- function(mut.parm){mut.parm@alleles.fn}
setGeneric("new.alleles")
not.equal.op <- function(mut.parm){mut.parm@not.equal.op}
setGeneric("not.equal.op")


chr.mutate <- function(chr, mutater, mutation.locations = NULL, mutations = NULL){
	chr; mutater; mutation.locations; mutations
	"generic base function does nothing"
}
...
setMethod("chr.mutate",
	signature = c("chromosome", "mutation.controller", "ANY", "ANY"),
	definition = function(chr, mutater, mutation.locations = NULL, mutations = NULL){
		chr.length <- length(chr)
		`%!=%` <- not.equal.op(mutater)
		if(is.null(mutation.locations))
			mutation.locations <- (runif(chr.length) < prob.mutation(mutater))
		genes <- chr[mutation.locations]
		if(length(genes) > 0){
			if(is.null(mutations))
				alleles <- new.alleles(mutater)(genes)
...
setMethod("chr.mutate",
	signature = c("list", "mutation.controller", "ANY", "ANY"),
	definition = function(chr, mutater, mutation.locations = NULL, mutations = NULL){
		n <- length(chr)
		returnValues <- vector("list", n)
		for(i in 1:n)
			returnValues[[i]] <- chr.mutate(chr[[i]], mutater, mutation.locations[[i]], mutations[[i]])
		new("returnList", returnValues)
	}
)

mutate.fgen <- function(mutation.controller){
	function(chr){chr.mutate(chr, mutation.controller)}
}
...
setMethod("evaluate", 
	signature = "list",
	definition = function(obj, fitness.fn, decode.fn, ...) {
		n <- length(obj)
...
fitness.chromosome <- function(chromosomes){
	fitVector <- vector("numeric", length(chromosomes))
	for(i in 1:length(chromosomes)){
		fitVector[i] <- chromosomes[i][["fitness"]]
	}
...
# 1. Control Parameters: max.gen, report.stats, environments  
# 2. Encode Environment:  chr.length, pop.size, new.population
#   |-- 2.1 Reproduction Environment: select.elite, select.chr, mutate, xover, xover.prob
#   |-- 2.2 Fitness Environment: fitness.fn, decode.fn, goal.fn, epsilon

setClass("ga.controller",
	representation(baseGA.env = "environment",
...
setMethod("initialize", 
	signature = "ga.controller",
   definition = function(.Object, ...) {
   	BaseGA.env <- new.env()
   	encoding.env <- new.env()
   	fitness.env <- new.env(parent = encoding.env)
   	reproduction.env <- new.env(parent = encoding.env)
   	BaseGA.env$encoding.env <- encoding.env
   	BaseGA.env$fitness.env <- fitness.env
   	BaseGA.env$reproduction.env <- reproduction.env

   	.Object@BaseGA.env <- BaseGA.env
   	.Object@encoding.env <- encoding.env
   	.Object@fitness.env <- fitness.env
   	.Object@reproduction.env <- reproduction.env
   }
)
...
generational.ga <- function(GA.env){
	with(GA.env, {
		...
		for(gen in 0:max.gen){
			...
			new.pop <- population(repr.results)
			pop <- new.pop
		}
	})
}
...
setup.chromosome.encoding <- function(GA.env, chr.encode.type){
	with(GA.env, {
...
create.mutation.controller <- function(GA, mutation.type){
	alleles.fn <- switch(mutation.type, 
		binary 		= alleles.binary,
		boolean 		= alleles.boolean,
		complement 	= alleles.fgen(alleles.complement, GA$gene.alphabet),
		spin 			= alleles.spin,
		symbolic 	= alleles.fgen(alleles.symbolic, GA$gene.alphabet),
		integer 		= alleles.fgen(alleles.integer, GA$gene.values),
		creap 		= alleles.fgen(alleles.creap, GA$gene.max, GA$gene.min),
		uniform 		= alleles.fgen(alleles.uniform, GA$gene.delta),
		gaussian 	= alleles.fgen(alleles.gaussian, GA$gene.sd),
	...
	new("mutation.controller", GA$prob.mutation, alleles.fn)
}

mutation.controller <- function(GA.env){GA.env$mutation.controller}
mutate.fn <- function(GA.env){GA.env$mutate}
...
choose.xover.swapMask <- function(GA, xover.type){
	xsm.fgen <- xover.swapMask.fgen
	switch(xover.type, 
		one.pt	= xover.mask.1point,
		two.pt	= xsm.fgen(xover.mask.kpoint, 2),
		k.pt 		= xsm.fgen(xover.mask.kpoint, GA$xover.k),
		uniform 	= xsm.fgen(xover.mask.uniform, GA$xover.alpha),
